...
class TestRunAgent:
    ...
    @patch("app.services.langchain_agent._get_agent")
    def test_run_agent_returns_last_message(self, mock_get_agent):
        """测试返回最后一条消息的内容"""
        mock_agent = MagicMock()
        msg1 = MagicMock()
        msg1.content = "中间回答"
    ...
    @patch("app.services.langchain_agent._get_agent")
    def test_run_agent_no_history(self, mock_get_agent):
        """测试不带历史消息的 Agent 调用"""
        mock_agent = MagicMock()
        mock_msg = MagicMock()
        mock_msg.content = "回答"
        ...
        result = run_agent("问题", history=None)
    ...
    @patch("app.services.langchain_agent._get_agent")
    def test_run_agent_invoke_error(self, mock_get_agent):
        """测试 Agent 调用出错时抛出异常"""
        mock_agent = MagicMock()
...
class TestRunAgentStream:
    ...
    @patch("app.services.langchain_agent._get_agent")
    async def test_run_agent_stream_yields_chunks(self, mock_get_agent):
    ...
    @patch("app.services.langchain_agent._get_agent")
    async def test_run_agent_stream_skips_empty_content(self, mock_get_agent):
    ...
    @patch("app.services.langchain_agent._get_agent")
    async def test_run_agent_stream_skips_non_message_events(self, mock_get_agent):
    ...
    @patch("app.services.langchain_agent._get_agent")
    async def test_run_agent_stream_skips_empty_event_data(self, mock_get_agent):
    ...
    @patch("app.services.langchain_agent._get_agent")
    async def test_run_agent_stream_with_history(self, mock_get_agent):
        """测试带历史消息的流式调用"""
        mock_agent = MagicMock()
        chunk = MagicMock()
        chunk.content = "回答"
        ...
        chunks = []
        async for c in run_agent_stream("新问题", history=history):
            chunks.append(c)

        assert len(chunks) == 1

    @patch("app.services.langchain_agent._get_agent")
    async def test_run_agent_stream_passes_correct_messages(self, mock_get_agent):
        ...
        async for _ in run_agent_stream("问题"):
            pass
